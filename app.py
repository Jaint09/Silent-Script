...
class Application:
    def __init__(self):
        ...
        self.root.bind('<Delete>', self.delete_last_char)   #for deleting character from the word
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            # Update Suggestions  --- FIXED PART ---
            if self.word:
                suggestions = self.spell_checker.candidates(self.word)
            else:
                suggestions = []
    ...
    def update_suggestions(self, predicts):
        # ---- FIX: handle None / empty ----
        if not predicts:
            predicts = []
        elif isinstance(predicts, set):
    ...
    def delete_last_char(self, event):
        ...
        Triggered by pressing the Delete key.
        """
        if self.word:
            # 1) current word se last character hatao
            self.word = self.word[:-1]

            # 2) sentence (self.str) me bhi last character hatao
            if self.str:
                self.str = self.str[:-1]

            # 3) GUI update
            self.panel4.config(text=self.word, font=("Courier", 40))  # Word label
            self.panel5.config(text=self.str, font=("Courier", 40))   # Sentence label

            print("Deleted last character. Current word:", self.word)
        else:
            print("No character to delete in current word.")
    ...
    def delete_last_word(self, event):
        ...
        if self.word:
            ...
            # 2) sentence (self.str) remove the last word 
    ...
    def action_call(self) :  # About section of the application
        ...
        self.photo1 = tk.PhotoImage(file='Picture/Lakshay_Jaint.png')
        self.w1 = tk.Label(self.root1, image = self.photo1)
        self.w1.place(x = 380, y = 105)
        self.tx1 = tk.Label(self.root1)
        self.tx1.place(x = 380,y = 250)
        self.tx1.config(text = "Lakshay Jaint", font = ("Courier",15,"bold"))
